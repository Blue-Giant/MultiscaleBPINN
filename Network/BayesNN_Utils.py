...
def build_lists(models, n_params_single=None, tau_priors=None, tau_likes=0.1, pde=False):
    if n_params_single is not None:  # Number of parameters for a single model
    ...
    if pde and build_tau_likes:        # tau_likes is a list
        tau_likes.append(tau_likes_elt)

    n_params = list(np.cumsum(n_params))   # cumulative sum [1,2,3] -> [1,3,6]
...
def define_model_log_prob_bpinns(models, model_loss, data, tau_priors=None, tau_likes=None, predict=False,
    ...
    def log_prob_func(params):
        ...
        if n_params_single is not None:
            ...
            for i in range(len(models)):  # converting a multi-dimensional structure into a one-dimensional structure
        ...
        l_prior = torch.zeros_like(params[0], requires_grad=True)  # initialization of a torch, log prior
        if tau_priors is not None:
            i_prev = 0
            for index, dist in zip(params_flattened_list, dist_list):  # param_single
                w = params[i_prev:index+i_prev]
                # print("**************W****************:", w)
                # print('**************log2W***********:', dist.log_prob(w))
        ...
        if predict:  # testing phase
            return (ll + l_prior/prior_scale), output
        else:
            return ll + l_prior/prior_scale  # just 1.0

    return log_prob_func


# added by li xi'an on 2024.05.27
def update_paras_by_pinn(models, data, model_loss, learning_rate=0.1, updatelr=True, step2change_lr=100,
                         gamma2change_lr=0.95, tau_priors=None, tau_likes=None, store_on_GPU=True, device='cpu',
                         n_params_single=None, pde=False,  total_epochs=10000, params_init_val=None):
    ...
    model_loss : function
        This determines the likelihood to be used for the model. You can customize this function in main code.
    learning_rate : float
        Size of each step to take when doing the numerical integration. Or say the step size for adjusting  the gradient of parameter
    tau_priors: float or list of float(s)
        Determines the stds of gaussian priors for parameters. If this is None then the priors become uniform distribution. If this is float then it becomes std of priors for all parameters. If this is a list then each element of the list becomes std of priors for [1st single parameter, 2nd single parameter,..., weights of 1st hidden layer, bias of 1st hidden layer, weights of 2nd hidden layer, bias of 2nd hidden layer,...]
    tau_likes: float or list of float(s)
        Data are assumed to be collected with gaussian noise and tau_likes determines the std of noise. If this is float then it becomes std of noise for all data. If this is a list then each element of the list becomes std of noise for each element of the list of models.
    ...
    pde : bool
        Determines whether it is pde or not.
    total_epochs : int      modify by li xi'an on 2023.11.14  epochs-->total_epochs
    ...
    losses: list of torch.tensor(s)
        A list of loss.
    """

    if n_params_single is not None:  # numb of param_single
    ...
    # The training data is fixed and used for all epoch of training process?? Yes
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=step2change_lr, gamma=gamma2change_lr)  # added by li xi'an on 2023.11.14
    for epoch in range(total_epochs):
        optimizer.zero_grad()
        loss = - log_prob_func(params)
        loss_list.append(loss)              # added by li xi'an on 2024.05.27
        loss.backward()
        optimizer.step()
        if updatelr is True:
            scheduler.step()                # added by li xi'an on 2023.11.14
        if epoch % 100 == 0:
            # modify by li xi'an on 2023.11.14
            print('Epoch: %d~~~~, loss: %.10f~~~~, learning rate: %.10f~~~~, time: %.2f' %
                  (epoch, loss.detach().item(), optimizer.param_groups[0]['lr'], time.time() - start_time))
    ...
    return list(map(lambda t: t.detach(), ret_params)), loss_list


# added by li xi'an on 2024.05.27
def update_paras_by_hamilton(models, data, model_loss, num_samples=10, num_steps_per_sample=10, learning_rate=0.1,
                        updatelr=True, step2change_lr=100, gamma2change_lr=0.95, burn=0, inv_mass=None, jitter=None,
                        normalizing_const=1., softabs_const=None, explicit_binding_const=100,
                        fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, jitter_max_tries=10,
                        sampler=hamiltorch.Sampler.HMC, integrator=hamiltorch.Integrator.IMPLICIT,
                        metric=hamiltorch.Metric.HESSIAN, debug=False, tau_priors=None, tau_likes=None,
                        store_on_GPU=True, desired_accept_rate=0.8, device='cpu', n_params_single=None,
                        pde=False, params_init_val=None):
    """Sample weights from a NN model to perform inference. This function builds a `log_prob_func` from the torch.nn.Module and passes it to `hamiltorch.sample`.
       Parameters
       ----------
       models : list of torch.nn.Module(s)
    ...
    if n_params_single is not None:  # num of param_single
        params_init = torch.zeros([n_params_single]).to(device)
    else:
        params_init = torch.Tensor([]).to(device)
    ...
    # The training data is fixed and used for all epoch of training process??
    log_prob_func = define_model_log_prob_bpinns(models, model_loss, data, tau_priors, tau_likes,
                                                 n_params_single=n_params_single, pde=pde)
    ...
    return hamiltorch.sample(log_prob_func, params_init, num_samples=num_samples,
                             num_steps_per_sample=num_steps_per_sample, step_size=learning_rate, update_lr=updatelr,
                             step2lr_update=step2change_lr, gamma2lr_update=gamma2change_lr, burn=burn,
                             jitter=jitter, inv_mass=inv_mass, normalizing_const=normalizing_const,
                             ...
                             jitter_max_tries=jitter_max_tries, sampler=sampler,
                             integrator=integrator, metric=metric, debug=debug,
                             desired_accept_rate=desired_accept_rate, store_on_GPU=store_on_GPU)


def sample_model_bpinns(models, data, model_loss, num_samples=10, num_steps_per_sample=10, learning_rate=0.1,
                        updatelr=True, step2change_lr=100, gamma2change_lr=0.95, burn=0, inv_mass=None, jitter=None,
                        normalizing_const=1., softabs_const=None, explicit_binding_const=100,
                        fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, jitter_max_tries=10,
                        sampler=hamiltorch.Sampler.HMC, integrator=hamiltorch.Integrator.IMPLICIT,
                        metric=hamiltorch.Metric.HESSIAN, debug=False, tau_priors=None, tau_likes=None,
                        store_on_GPU=True, desired_accept_rate=0.8, device='cpu', n_params_single=None,
                        pde=False, pinns=False, total_epochs=10000, params_init_val=None):
...
def predict_model_bpinns(models, samples=None, data=None, model_loss=None, tau_priors=None, tau_likes=None,
                         pde=False, n_params_single=None):
