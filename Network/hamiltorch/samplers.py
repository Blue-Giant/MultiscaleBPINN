...
def leapfrog(params, momentum, log_prob_func, steps=10, step_size=0.1, jitter=0.01, normalizing_const=1.,
             softabs_const=1e6, explicit_binding_const=100, fixed_point_threshold=1e-20, fixed_point_max_iterations=6,
             jitter_max_tries=10, inv_mass=None, ham_func=None, sampler=Sampler.HMC, integrator=Integrator.IMPLICIT,
             metric=Metric.HESSIAN, store_on_GPU=True, debug=False, pass_grad=None):
...
def sample(log_prob_func, params_init, num_samples=10, num_steps_per_sample=10, step_size=0.1, update_lr=True,
           ...
           store_on_GPU=True, pass_grad=None, verbose=False):
    ...
    for n in range(num_samples):
        ...
        try:
            momentum = gibbs(params, sampler=sampler, log_prob_func=log_prob_func, jitter=jitter,
                             normalizing_const=normalizing_const, softabs_const=softabs_const, metric=metric, mass=mass)
            ...
            if sampler == Sampler.RMHMC and integrator == Integrator.EXPLICIT:
                ...
                new_ham = rm_hamiltonian(params, momentum, log_prob_func, jitter, normalizing_const,
                ...
                new_ham = hamiltonian(params, momentum, log_prob_func, jitter=jitter, softabs_const=softabs_const,
                                      explicit_binding_const=explicit_binding_const,
...
def define_split_model_log_prob(model, model_loss, train_loader, num_splits, params_flattened_list, params_shape_list,
                                tau_list, tau_out, normalizing_const=1., predict=False, device='cpu', verbose=False):
...
def sample_model(model, x, y, params_init, model_loss='multi_class_linear_output', num_samples=10,
                 num_steps_per_sample=10, step_size=0.1, burn=0, inv_mass=None, jitter=None, normalizing_const=1.,
                 ...
                 integrator=Integrator.IMPLICIT, metric=Metric.HESSIAN, debug=False, tau_out=1., tau_list=None,
                 store_on_GPU = True, desired_accept_rate=0.8, verbose=False):
    ...
    return sample(log_prob_func, params_init, num_samples=num_samples, num_steps_per_sample=num_steps_per_sample,
                  step_size=step_size, burn=burn, jitter=jitter, inv_mass=inv_mass, normalizing_const=normalizing_const,
                  ...
                  jitter_max_tries=jitter_max_tries, sampler=sampler, integrator=integrator, metric=metric,
                  debug=debug, desired_accept_rate=desired_accept_rate, store_on_GPU=store_on_GPU, verbose=verbose)


def sample_split_model(model, train_loader, params_init, num_splits, model_loss='multi_class_linear_output',
                       num_samples=10, num_steps_per_sample=10, step_size=0.1, burn=0, inv_mass=None, jitter=None,
                       normalizing_const=1., softabs_const=None, explicit_binding_const=100, fixed_point_threshold=1e-5,
                       fixed_point_max_iterations=1000, jitter_max_tries=10, sampler=Sampler.HMC,
                       integrator=Integrator.SPLITTING, metric=Metric.HESSIAN, debug=False, tau_out=1.,
                       tau_list=None, store_on_GPU=True, desired_accept_rate=0.8, verbose=False):
...
def predict_model(model, samples, x=None, y=None, test_loader=None, model_loss='multi_class_linear_output',
                  tau_out=1., tau_list=None, verbose=False):
