...
def save_bayes_net2file_with_keys(outPath=None, model2net=None, paras2net=None, name2model='DNN', learning_rate=None,
                                  expected_log_prob=0.1, epoch=100, opt2update_para='PINN'):
    """
    Args:
         outPath: the output path for saving net model
         model2net: the net model required to save
         paras2net: the parameters for net
         name2model: the name of network
         optimizer: the optimizer, such as Adam, Moment, SGD....
         scheduler: a scheduler used to adjust learning rate for optimizer
         epoch: current training epoch
    ...
    torch.save(
        {'epoch': epoch,
         ...
         'opt2update_para': opt2update_para}, outFile_with_path)


def load_bayes_net2file_with_keys(path2file=None, model2net=None, name2model='DNN'):
    """
    Args:
        path2file: the path for loading net model
        model2net: the net model required to load
    ...
    checkpoint = torch.load(file2model_with_path)
    model2net.load_state_dict(checkpoint['model'])
    paras_hmc = checkpoint['paras']
    probability = checkpoint['expected_log_probability']
    opt2update_para = checkpoint['opt2update_para']
    lr = checkpoint['learning_rate']
    return paras_hmc, probability, opt2update_para, lr
