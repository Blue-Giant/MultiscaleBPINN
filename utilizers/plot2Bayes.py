...
def plot_scatter_solution2test(solu2test, test_xy=None, name2solu='Solu', outPath=None):
...
def plot_scatter_force2test(solu2test, test_xy=None, name2solu='Force', outPath=None):
    test_x_bach = np.reshape(test_xy[:, 0], newshape=[-1, 1])
    test_y_bach = np.reshape(test_xy[:, 1], newshape=[-1, 1])
    ...
    # plt.title('solution', fontsize=15)
    fntmp = '%s/force2%s' % (outPath, name2solu)
    DNN_tools.mySaveFig(plt, fntmp, ax=ax, isax=1, iseps=0)
