...
def generate_points(num2points=1000, sigma=0.1, lam1=0.1, lam2=0.1, lb=0.0, rb=1.0):
    X = np.linspace(lb, rb, num2points)[:, None]
...
class PINN2Possion(torch.nn.Module):
    def __init__(self, in_dim=1, out_dim=1, hiddens=None, Model_name='DNN', actName2In='tanh', actName2Hidden='tanh',
        ...
        if 'NET_2HIDDEN_FF' == str.upper(Model_name):
            ...
            self.fnn = DNN2Bayes.Net_2Hidden(layer_sizes, actName2in=torch.tanh, sigma=1.0, trainable2sigma=False,
                                             type2float='float32', to_gpu=False, gpu_no=0)

        elif 'NET_3HIDDEN' == str.upper(Model_name):
            layer_sizes = np.concatenate([[1], hiddens, [1]], axis=0)
            self.fnn = DNN2Bayes.Net_3Hidden(layer_sizes, actName2in=torch.tanh, sigma=1.0, trainable2sigma=False,
                                             type2float='float32', to_gpu=False, gpu_no=0)
...
X_train, y_train = generate_points(num2points=train_num, sigma=sigma, lam1=lam1, lam2=lam2, lb=0.0, rb=1.0)
X_test, y_test = generate_points(num2points=test_num, sigma=0, lam1=lam1, lam2=lam2, lb=0.0, rb=1.0)
