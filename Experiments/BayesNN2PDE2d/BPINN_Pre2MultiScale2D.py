...
def solve_bayes(Rdic=None):
    ...
    rel_square = np.sqrt(np.mean(np.square(solu2test-u_val), axis=0)/np.mean(np.square(u_val), axis=0))
    ...
    plot2Bayes.plot_scatter_solution2test(solu2test, test_xy=x_val, name2solu='BPINN_Pre', outPath=R['FolderName'])
    plot2Bayes.plot_scatter_solution2test(u_val, test_xy=x_val, name2solu='Exact_Pre', outPath=R['FolderName'])
    plot2Bayes.plot_scatter_solution2test(point_abs_errs, test_xy=x_val, name2solu='Absolute_Error_Pre',
...
if __name__ == "__main__":
    ...
    date_time_dir = '5m_27d_17h_24m_35s'
    ...
    if R['mode2update_para'] == 'PINN':
        ...
        R['learning_rate'] = 0.000025  # this is the learning rate for optimizer in Hamilton  model
        R['step2update_lr'] = 25
        # R['gamma2update_lr'] = 0.95
